...
class PlanetWars(PlanetWarsProxy):
    ...
    def Tick(self):
        ...
        for p in arrivals:
            ...
            if (max == second):
                ...
                if (p.Owner() == owner):
                    if len(forces) == 1:
                        self.logger("{0:4d}: Player {1} reinforced planet {2}".
                                    format(self._tick, owner, p.ID()))
                    else:
                        self.logger("{0:4d}: Player {1} defended planet {2}".
                                    format(self._tick, owner, p.ID()))
                else:
                    p.Owner(owner)
                    self.logger("{0:4d}: Player {1} now owns planet {2}".
                                format(self._tick, owner, p.ID()))
    ...
    def EnemyFleets(self, player_id):
        ...
        for f in self._fleets:
            fleet = self._fleets[f]
            if (fleet.Owner() == player_id) or (
                    fleet.Owner() == PlanetWarsProxy.NEUTRAL_PLAYER):
                continue
            r.append(fleet)
        return r
    ...
    def ProcessOrders(self, player_id, orders):
        for order in orders:
            ...
            numships = order[3]
            fleet = Fleet(new_id, player_id, numships,
                          source.X(), source.Y(), dest)
            self.logger(
                "{0:4d}: Player {1} launched {2} (of {3}) ships from planet {4} to planet {5}".
                format(self._tick, player_id, numships,
                       source.NumShips(), source.ID(), dest.ID()))
